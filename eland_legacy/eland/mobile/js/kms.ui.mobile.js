	...
	if(!body.hasClass('isLyr')){
		...
		if(st > hdrH){
			if(body.hasClass('scrollDown')){
				...
				if(($('.ctgNavWrap').length>0 && st > $('.ctgNavWrap').offset().top - hdr.outerHeight(true))  || ($('.shpOften_wrap').length>0 && st > $('.shpOften_wrap').offset().top - hdr.outerHeight(true))   || ($('.catg_sel.plan_sel').length>0 && st > $('.catg_sel.plan_sel').offset().top - hdr.outerHeight(true))  ){
					hdr.css({'transform':'translate3d(0,-'+hdrH+'px,0)'})
					body.addClass('sdHdr')
				}else{
