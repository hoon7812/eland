		...
		**/	
		...
		setBackParameters : function(parameters){
			...
			for(i=0;i<params.length;i++){
				...
				// 보기갯수
				if(name == "pageSize"){
					$("#hstr_pageSize").val(val);
					$(".set_lp_option .sel_btn .sel_txt").attr('data-sel-msg', val + '개씩 보기');
					$(".set_lp_option .sel_btn .sel_txt").html('<span>'+val+'</span>개씩 보기');
				}
			...
			if(typeof(max_price) != undefined && max_price > 0 ){
				...
				if($('#l_range').length > 0){
					if(typeof priceChange == 'function'){
						priceChange();
					}else{
						slideRngChange();
					}
