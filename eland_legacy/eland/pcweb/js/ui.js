...
commonUI.dimCallBlk = commonUI.dimCallBlk || {};
commonUI.dimRemoveBlk = commonUI.dimRemoveBlk || {};
commonUI.dblOpen = commonUI.dblOpen || {};
commonUI.dblClose = commonUI.dblClose || {};

commonUI.contTest = commonUI.contTest || {};


commonUI.favSch = commonUI.favSch || function(){}; // 단골매장 검색 관련
commonUI.controlSlide = commonUI.controlSlide || function(){}; // 슬라이드 좌우 버튼 컨트롤
...
commonUI.view.slideConer=function($target){ // 공통 Slide 2018.08.23 JEC@pionnet.co.kr
	...
	$($target)[0].options={
		...
		loop : commonUI.view.slideConer.prototype.TorF($($target)[0].opt.loop),
		// loop : true,
		loop : true,
...
commonUI.view.slideConer.prototype.TorF = function(param){
	if(param == 'false'){
		return false;
	}
	else{
		...
		var $parent = $(this).parent('.lyr_select');
		 if(!$(this).parent().hasClass('disabled')){
			if ($(this).parent().hasClass('on')){
				...
				if($(this).parent().hasClass('hasDefault') && $(this).parent('.hasDefault').find('.pSel').length > 0 )  { //해당상품 seleced 후 스크롤 이동 //NGCPO-6522
					var $li = $(this).parent().parent();
...
//주얼리 호수 옵션 스크롤 가운데로 디폴트_상세상단 옵션영역에만 적용
...
//기획전 선택된 옵션 스크롤 맨위로_상세플로팅과 퀵뷰팝업에 적용 200406이정은
commonUI.view.lyrSlt.prototype.goToSelect = function($li){ //해당상품 seleced 후 스크롤 이동 건 //NGCPO-6522 [FO] 럭키딜 상세페이지 내 구매하기 상품리스트 개선(버그)
	var $scroller = $li.find('.options'); //.option 스크롤러
	$scroller.scrollTop(0); //정확한 좌표 측정을 위해 스크롤을 상단으로 초기화
	var scrollerOfst = $scroller.offset().top; //스크롤러 상단 좌표 
	var defualtVOfst =$scroller.find('.pSel').offset().top; ////select옵션 상단 좌표
	var gap = defualtVOfst - scrollerOfst; //
	var optHeight = $li.find('.options li').height(); //옵션 높이값
	var moveRange_P =  gap - ( $scroller.find('.pSel').height() - optHeight ); 
	$scroller.scrollTop(moveRange_P);	
}
	...
	function imgsrcInit(){
		$lpItem.each(function(){
			...
			if ( $Links.attr("data-imgsrc") != undefined ) {
				var _vals = {
					$imgTarget : $this.find(".thumb img").not($(".flag img")) //NGCPO-7172 플래그 호버효과 미적용
...
function lnb_depth_size(){
	$('#cateMainLnb').find('.depth2').each(function(){
		...
		if($(this).find('div').hasClass('lb03')){
			$(this).css({'width':'330px','padding-bottom':'196px'});
			if($(this).find('.swiper-slide').length>1){
				var bnr = $(this).find('.swiper-container')
				var idx = bnr.attr('id')
				var idx = new Swiper('#'+idx,{
					loop : true,
					autoplay : 3000,
					speed : 300,
					pagination : bnr.find('.roll_indi')[0],
					paginationElement : "a" ,
					paginationClickable : true,
					simulateTouch : true
				})
				idx.stopAutoplay();
				$(this).parent('li:first').hover(
					function(e){
						idx.startAutoplay();
					},
					function(e){
...
commonUI.dimCallBlk = function(){
	$('<div class="blk_dim"></div>').insertAfter($('body'));
}

commonUI.dimRemoveBlk = function(){
	$('.blk_dim').remove();
}

commonUI.dblOpen = function(id){
	commonUI.dimCallBlk();
	$(id).show();
}

commonUI.dblClose = function(id){
	commonUI.dimRemoveBlk();
	$(id).hide();
}
...
commonUI.controlSlide = function($target){
	$parentBox = $($target.container).parent();
	$leftBtn = $parentBox.find('.btn-prev2');
	$rightBtn = $parentBox.find('.btn-next2');
	$firstChild = $($target.slides[0]);
	$lastChild = $($target.slides.slice(-1)[0]);
	if($firstChild.hasClass('swiper-slide-visible')){
		$leftBtn.removeClass('on');
	}
	else if(!$firstChild.hasClass('swiper-slide-visible')){
		$leftBtn.addClass('on');
	}

	if($lastChild.hasClass('swiper-slide-visible')){
...
$(document).ready(function(){
	if($('.new_cate_wrap .l_content .box_filter').length>0 && !($('div').hasClass('kims'))){//킴스제외처리
		scrollCheck(0,0)
		$('.l_content').addClass('fixed')
		$('.l_content, .r_content').wrapAll('<div class="sr_cont"></div>')
		$('.r_content').css({'min-height':$('.l_content').outerHeight(true)+'px'})
		if($('.rsch_kwd').length>0){
			fnFilterFixed('.l_content', '.sr_cont', 46)
		}else{
			fnFilterFixed('.l_content', '.sr_cont', 67)
		}
	}
})

function fnFilterFixed(fixed_selector, wrap_selector, btm_margine){
	$(fixed_selector).each(function(){
		...
		function set_pos(){
			...
			var ctn_top = wrap.offset().top
			var ctn_limit = wrap.outerHeight(true) - winHeight
			var ctn_spc = ctn_limit + ctn_top

			if (winScr > ctn_top - btm_margine && get_fxEl_height() < wrap.outerHeight(true) ) {
				if(get_fxEl_height() > winHeight){
					if(winScr > ctn_spc){
						fixedEl.css({'position':'absolute','bottom':'auto','top':wrap.outerHeight(true)-get_fxEl_height()})
					}else{				
						if($('body').hasClass('scrollDown')){
							if(winScr < get_fxEl_top() + get_fxEl_height() - winHeight -1){
								fixedEl.css({'position':'absolute','bottom':'auto','top':get_fxEl_top()-ctn_top+'px'})
							}else{
								fixedEl.css({'position':'fixed','top':'auto','bottom':0+'px'})
							}	
						}else{
							if(winScr <= get_fxEl_top() - btm_margine){
								fixedEl.css({'position':'fixed','top': btm_margine+'px','bottom':'auto'})
							}else{
								fixedEl.css({'position':'absolute','bottom':'auto','top':get_fxEl_top() -ctn_top+'px'})
							}
						}
					}
				}else{ //option remove active
					if(winScr >= ctn_spc - btm_margine + winHeight - get_fxEl_height()){
						fixedEl.css({'position':'absolute','bottom':'auto','top':wrap.outerHeight(true)-get_fxEl_height()+'px'})
					}else{
						fixedEl.css({'position':'fixed','top': btm_margine+'px','bottom':'auto'})
					}
				}
			}else{
				fixedEl.css({'position':'absolute','bottom':'auto','top':mrg+'px'})
			}
			return false
		}
		function get_fxEl_top(){
			return fixedEl.offset().top
		}
		function get_fxEl_height(){
			return fixedEl.outerHeight(true)
		}
	})
}
function scrollCheck(deltaInt ,intervalInt){
	var winScroll
	var lastScroll = 0
	var delta = deltaInt
...
$(document).ready(function(){
	if($('.ctg_tab_wrap').length>0){
		$('#wrapper').addClass('cateFx')

		$bt_prev=$('.btn_ctr .prev');
		$bt_next=$('.btn_ctr .next');
		if($('.ctg_tab').find('.swiper-slide').length > 10){
			setCtgTab();
		}else{
			$('.ctg_tab_wrap .btn_ctr').remove();
			//brandRolling =  false;
		}
		function setCtgTab(){
			brandRolling = new Swiper('.ctg_tab .swiper-container',{
				slidesPerView : 10,
				simulateTouch : false,
				cssWidthAndHeight: true,
				onSlideChangeEnd : function() {
					ctgTab_btn();
					//if(brandRolling.scrollFlag==true) fnScroll()
				},
				onSwiperCreated : function(){
					ctgTab_btn();
				}
			});
			//brandRolling.scrollFlag=false;
			$bt_prev.on('click', function(e){ e.preventDefault();brandRolling.scrollFlag=false;brandRolling.swipePrev() });
			$bt_next.on('click', function(e){ e.preventDefault();brandRolling.scrollFlag=false;brandRolling.swipeNext() });
		}
		function ctgTab_btn(){
			
			var total = $('.ctg_tab .swiper-slide').length;
			console.log(total, $('.ctg_tab .swiper-slide-active').index())

			if($('.ctg_tab .swiper-slide-active').index()==0){
				$bt_prev.removeClass('active');
			}else{
				$bt_prev.addClass('active');
			}
			if($('.ctg_tab .swiper-slide-active').index() + 10 >= total){
				$bt_next.removeClass('active');
			}else{
				$bt_next.addClass('active');
			}
		}
