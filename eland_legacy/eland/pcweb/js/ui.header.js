...
commonUI.header.mallToggleLyr = function(){
	...
	if($("div").hasClass("plan_makeup")){ // 160901
		...
		if($.inArray(brd_Class, ['modern','modern xmas','kinder','folder','mixxo','ost','eblin','newbal','roem','lloyd','clue','spao','whoau','shoopen','bahus','nb_kids','kims','renewal']) >= 0){ //add renewal
	...
	function headerFixFunc(){
		...
		if($('.ctg_tab_wrap').length>0){
			cateTabOfst = $('.ctg_tab_wrap').eq(0).offset().top - $('#content').offset().top;
			if ( $(window).scrollTop() < $('.ctg_tab_wrap').eq(0).offset().top ) {
				_originals();
			} else {
				_minimal();
			}
		}else{
			if ( $(window).scrollTop() < LIMIT - top_bn_height ) {
				_originals();
			} else {
				_minimal();
			}
...
commonUI.header.catgToggleLyr = function(){
	...
	var $btnV;
	var $btn = $(".catg_bt");
	var $btnOver = $(".luxury .catg_bt, .luxury .brand_bt, .kims .catg_bt");
	var $outRange = $('.catg_wrap, .brd_wrap');
	...
	$btn.not($btnOver).on("click" ,function(e){
		$btn.toggleClass("on");
		if ( $btn.hasClass("on") ) {
			gnbMenu.offEvtL();
	...
	$btnOver.on('mouseenter' ,function(e){
		$btnV = $(this);
		$targetV = $(this).find('+.catg_lyr')
		$btnV.addClass("on");
		$targetV.show();
	});
	$outRange.on('mouseleave',function(){
		$btnV.removeClass("on");
		$targetV.hide();
	});
...
commonUI.header.schHeader = function(){
	...
	function schHeaderEventHandler(){
		...
		$links.each(function() {
			var $this = $(this).not('.noInput li a');
...
console.log(hd_banner_opt.mode)
...
commonUI.header.gnbCate.prototype = {
	...
	depth1On : function(idx) {
		// if(this.$depth1.eq(idx).find(".depth2").length > 0 && $.trim(this.$depth1.eq(idx).find(".depth2").html()) != "") {
...
$(document).ready(function(){
	...
	if($('#wrapper').hasClass('spao') || $('#wrapper').hasClass('bahus') || $('#wrapper').hasClass('mixxo')){
		hd_disp_line_up()
	}
})
