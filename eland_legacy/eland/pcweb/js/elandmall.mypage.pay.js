					...
					callback: function(data) {
						...
						$.ajax({	//주문 데이타 생성
							...
							success: function(data) {
								if(data.ret_code=='S'){
									location.href = elandmall.util.https("/mypage/initMyOrderCancelResult.action?claim_no="+data.claim_no+"&ord_no="+data.ord_no+"&claim_cd="+data.claim_cd);
								}
							},
							error: function(m) {
								var message = "접수중 오류가 발생했습니다.";
								if (m.error_type == "kr.co.overpass.exception.UserException") {
