		 ...
		 if(!$(this).parent().hasClass('disabled')){
			if ($(this).parent().hasClass('on')){
				if($parent.hasClass('multi')){
					// commonUI.view.lyrSlt.prototype.multiTxt($(this))
				}
				...
				if($parent.hasClass('multi')){
					console.log('siblings do something')
				}
...
commonUI.view.lyrSlt.prototype.multiTxt = function($target){
	var $options = $target.siblings('.lyr_options');
	var $btnTxt = $target.find('.sel_txt');
	var finTxt;
	var $on = $options.find('.ancor.on')
	$options[0].multiTxt = [];
	$on.each(function(){
		var txt = $(this).find('.opt_name').html();
		$options[0].multiTxt.push(txt);
	})
	finTxt = $options[0].multiTxt.join(", ")
	console.log(finTxt)
	//선택 항목이 없을 경우 케이스 추가 필요.
	$btnTxt.html(finTxt);
}
...
commonUI.tabSlide = function(target){
	...
	var doNext = function(){
		if($('#'+target+' li.on:last-child').length){
			$('#'+target+' li:first-child a').click();
		}
		else{
			$('#'+target+' li.on + li a').click();
		}
	}
	init();
}
