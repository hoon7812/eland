$(document).ready(function() {
	...
	$("#regist_order_button").click(function() {	//구매하기 버튼 클릭
		...
		if( $("a[name=ord_pay_radio][pay_mean_cd=14]").hasClass("selected")
		   || $("a[name=ord_pay_radio][pay_mean_cd=11][payco=Y]").hasClass("selected")
		   || $("a[name=ord_pay_radio][pay_mean_cd=11][toss=Y]").hasClass("selected")
		   || $("a[name=ord_pay_radio][pay_mean_cd=11][naverpay=Y]").hasClass("selected")){ //NGCPO-9381 휴대폰결제, 토스, 네이버페이, 페이코 결제 시 딤드 처리 추가
			// dimm on
		...
		$.ajax({
				...
				success: function(result) {
					...
					if(pointInfo.elandPoint < 5000){
						...
						$("#point_row").append("<p class='txt_alert_ord01' id='point_txt'>*  <strong class='txt_en'>5,000</strong>원부터 사용가능</p>");
					}else{
	...
	$("#certNoSendBtn,#reCertNoSendBtn").click(function() {
		...
		if(this.id == "certNoSendBtn"){
			 type = $('[name="pnt_rdo"]').filter(":checked").attr("data-value");
		} else {
			 type = $('[name="pnt_rdo2"]').filter(":checked").attr("data-value");
		}
