				...
				if (p.logout && $.type(p.logout) == "function") {
			    	...
			    	//로그아웃 후 메인으로 이동 처리 _ 슈펜의 경우 슈펜메인으로 이동 그외 통합몰 메인으로 이동
			    		...
			    		if(elandmall.global.disp_mall_no == "0000053"){
			    			http_url = "http:" + elandmall.global.shoopen_domain_url;
			    		}
			    	window.location.href = elandmall.util.http(http_url+"/main/initMain.action");
				...
				if (data.login) {
					...
					if(elandmall.global.chnl_cd == '40' && $.type(elandmall.global.app_version) != "undefined" && elandmall.app.elandApp(elandmall.global.app_version)){
						...
						// APP에서 웹뷰 호출 ==> /app/initAppHeader.action?path_url=/login/initLogin.action?gnbwebview=Y 하면/fo_mobile/webapp/main/api/initAppHeader.jsp 페이지 노출 is_app true로 셋팅됨
							...
							location.href=elandmall.util.http("/app/initAppHeader.action?path_url=/login/initLogin.action&gnbwebview=Y");
							return false;
						}
					}
	...
	elandmall.loginProc = {
		...
		fnOrdLogin : function(formObj , pin) {
			...
			if(location.protocol == "https:"){
				...
				form.submit({
		            iframe: true,
		            action: elandmall.util.https("/login/loginProc.action"),
		            success: function(p) {
			...
			//http => https 인경우는 도메인이 다르므로 아이프레임 hidden으로 처리한다.	
				...
				formObj.attr("action", elandmall.util.https("/login/initLoginLayerProc.action"));
				formObj.submit();
			}
		},
		        				...
		        				if(elandmall._login_callback != null) {	
				    				...
				    				if (result.return_url != null) {
				    				    if (result.return_url.indexOf("http") == 0) {
				    				        ...
				    				        f.attr("action", elandmall.util.http(result.return_url));
				    				    };  
				    				} else {
				    					if(elandmall.global.disp_mall_no == "0000053"){
				    						f.attr("action", elandmall.util.http("http:"+ elandmall.global.shoopen_domain_url + "/main/initMain.action"));
				    					} else {
				    						f.attr("action", elandmall.util.http("/main/initMain.action"));
				    					}
				    				};                  
