...
*/
	...
	elandmall.shoopen = {
		...
		goCurationRanking : function(large_disp_ctg_no) {
			if($.type(large_disp_ctg_no) !="undefined" && large_disp_ctg_no != null && large_disp_ctg_no != ""){
				location.href = elandmall.util.newHttps("https:" + elandmall.global.shoopen_domain_url + "/shop/initShoopenCuration.action?ranking_tab=Y"+"&large_disp_ctg_no="+large_disp_ctg_no);
			}else{
		...
		// 슈펜 신상품매장으로 이동
		goShoopenNewGoods : function(large_disp_ctg_no,new_type){
			if($.type(new_type) != "undefined" && new_type != null){ // PC
				if($.type(large_disp_ctg_no) !="undefined" && large_disp_ctg_no != null && large_disp_ctg_no != ""){
				...
				if($.type(large_disp_ctg_no) !="undefined" && large_disp_ctg_no != null && large_disp_ctg_no != ""){
					location.href = elandmall.util.newHttps("https:" + elandmall.global.shoopen_domain_url + "/shop/searchNewGoodsJson.action?large_disp_ctg_no="+large_disp_ctg_no);
				}else{
					location.href = elandmall.util.newHttps("https:" + elandmall.global.shoopen_domain_url + "/shop/searchNewGoodsJson.action");
				}
			}
		},
		// 슈펜 sale매장으로 이동
		...
		// 슈펜 best매장으로 이동
		goShoopenBestGoods : function(large_disp_ctg_no,best_type){
			if($.type(best_type) != "undefined" && best_type != null){ // PC
				if($.type(large_disp_ctg_no) !="undefined" && large_disp_ctg_no != null && large_disp_ctg_no != ""){
					location.href = elandmall.util.newHttps("https:" + elandmall.global.shoopen_domain_url + "/shop/initShopBest100.action?disp_lctg_no="+large_disp_ctg_no);
				}else{
	...
	elandmall.custcenter = {
		link : {
			// 메인화면 링크
			main : function() {
				location.href = elandmall.util.newHttps("/custcenter/initCustCenterMain.action");
			},
			// 상담 문의 링크
			...
			notice : function(noti_no, search_type, disp_mall_no, main) {
				...
				if(disp_mall_no == "0000053") {
					location.href = "https:" + elandmall.global.shoopen_domain_url + "/custcenter/initCustNoticeList.action?search_type=" + search_type + param;
				} else {
					location.href = "https:" + elandmall.global.base_domain_url + "/custcenter/initCustNoticeList.action?search_type=" + search_type + param;
				}
			},
			// 복지혜택
			welfare : function() {
	...
	elandmall.shoopen.mypage = {
		...
		link : {
			...
			modifyMemberInfo : function(login_id , membState, returnUrl){  //회원정보 수정 , 90일 변경, 임시 비밀번호
				...
				if($.type(returnUrl) == "undefined" ){
					returnUrl = 'https:'+elandmall.global.base_domain_url+'/gate/memGate.action?entry_url='+elandmall.global.https_url+'/mypage/shoopen/initMypageMain.action';
				}
				...
				elandmall.oneclick.goPage("/member/memberOut",
					{
						siteCode : '10',
						webId : login_id,
						returnUrl : 'https:'+elandmall.global.base_domain_url+'/gate/memberOutGate.action'},
					null,
					"post");
			},
			manageSnsAccount : function(login_id , membState, returnUrl){  // SNS 연동관리
