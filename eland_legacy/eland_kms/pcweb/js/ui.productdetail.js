...
commonUI.view.detail.FullZoom.prototype = {
	...
	,zoomLayerOpen : function(){
		...
		// $("body ,html").addClass("hidden").animate({"scrollTop":"100px"},10);
	...
	,thumInit : function(){
		...
		if ( this.$thumPageCnt < 1 ) // 썸네일 한페이지 이하 스크립트 정지
		{
			this.$thumBtnLeft.remove();
			this.$thumBtnRight.remove();
			// return false;
		}
		this.thumEvent();
...
commonUI.view.detail.tabFocus = function(e){
	...
	$links.on( "click" , function(e){
		...
		var _idTop = parseInt($thisID.offset().top) - 56; //160810 수정
