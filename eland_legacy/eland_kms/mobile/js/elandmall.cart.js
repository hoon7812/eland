;(function($) {
	...
	$(document).ready(function() {
		...
		var all_check_button = $("a[name='all_check_button']");
		var div_cart_con = $("div.cart_con");
		var tab_cart_a = $("ul.tab_cart a");
			...
			updateCartList = function() {
				...
				$.each(CART.groups, function(cart_grd_cd) {
					...
					$.each(this, function(key) {	//key: dlvp_seq, vir_vend_no
						...
						if ((cart_grd_cd == "10" || cart_grd_cd == "60" || cart_grd_cd == "70") && deli.r_deli_cost_form_cd != "30" && deli.r_deli_cost_form_cd != "60" && sub_goods_price_sum > 0 && deli.st_amt > sub_goods_price_sum_for_deli) {	//택배 배송에 착불/묶음무료가  아니면 배송비 노출
							total_deli_price += deli.deli_cost_amt;
						};						
				...
				var checkCarts = function(soldout) {
					...
					if (soldout === true) {
						$("input:checkbox[role='item_check'][area='soldout']:checked").each(function() {
							...
							var goods = CART.goods[cart_seq];
							sold_out_count++;
							sold_out_nm += ((sold_out_count != 1 ? ", " : "") +  goods.disp_goods_nm);
						});						
					};
					if (cart_no.length == 0) {
		...
		(function() {
			...
			$("a[name='delete_button']").click(function(e) {
				var button = $(this);
		...
		$("a[name='goods_detail']").click(function(e) {
			...
			if (cart.set_goods_no != "" && cart.set_items.length == 0) {	 //묶음상품
				elandmall.goods.goDetail({ goods_no: cart.set_goods_no });
			} else {
				elandmall.goods.goDetail({ goods_no: cart.goods_no, vir_vend_no: cart.vir_vend_no });				
			};
			return false;
		});
